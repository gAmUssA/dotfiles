...
#
# Signals:
#   1. Terminal BEL via /dev/tty  → tmux `visual-bell` flashes the status bar
#   2. Desktop notification via alerter → macOS Notification Center banner
...
# 1. Terminal bell — straight to the controlling tty so tmux sees it
printf '\a' > /dev/tty 2>/dev/null

# 2. Build notification subtitle from cwd + git branch
...
# 3. Fire the notification. Background the subshell so the hook returns fast.
